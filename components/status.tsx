  ...
  return (
    <ul className="grid grid-cols-1 grid-rows-none gap-4 md:grid-cols-12 md:grid-rows-3 lg:gap-4 xl:max-h-[34rem] xl:grid-rows-2 mb-10">
      <GridItem
        ...
        icon={<BookOpenText className="h-4 w-4 text-black dark:text-neutral-400" />}
...
const GridItem = ({ area, icon, title, description }: GridItemProps) => {
  return (
    <li className={`min-h-[14rem] list-none ${area} `}>
      <div className="relative h-full rounded-2.5xl border  p-2  md:rounded-3xl md:p-3 hover:shadow-lg transition-shadow duration-500 ">
        <GlowingEffect
          spread={40}
          glow={true}
          disabled={false}
